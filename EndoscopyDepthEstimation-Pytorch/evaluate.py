    ...
    if phase == "validation":
        ...
        with torch.no_grad():
            ...
            for batch, (colors_1, boundaries, intrinsics, names) in enumerate(test_loader):
                ...
                cv2.imwrite("image/color/" + "{}.png".format(names[0]), color_display)
                np.save("image/depth/" + "{}.npy".format(names[0]), depth_map)
                # cv2.imwrite("image/depth/" + "{}.png".format(names[0]), depth_display)
                tq.update(batch_size)
