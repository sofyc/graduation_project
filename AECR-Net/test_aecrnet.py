...
def customer_test(net, path):
	ckp = torch.load('./pretrained_model/'+opt.pre_model)
	# ckp['model']["module.dcn_block.dcn.conv_offset.weight"] = ckp['model']["module.dcn_block.dcn.conv_offset_mask.weight"]
	# ckp['model']["module.dcn_block.dcn.conv_offset.bias"] = ckp['model']["module.dcn_block.dcn.conv_offset_mask.bias"]
	# ckp['model'].pop("module.dcn_block.dcn.bias")
	# ckp['model'].pop("module.dcn_block.dcn.conv_offset_mask.weight")
	# ckp['model'].pop("module.dcn_block.dcn.conv_offset_mask.bias")
	# print([k for k, v in ckp['model'].items()])
	# exit()
	weights = collections.OrderedDict([(k[7:], v) for k, v in ckp['model'].items()])
	net.load_state_dict(weights)
	net.eval()
	torch.cuda.empty_cache()
	a = torch.tensor(cv2.imread(path)).float()/256
	print(a)
	print(torch.max(a))
	print(torch.min(a))
	print(a.shape)
	inputs = ((torch.tensor(cv2.imread(path)).float())/256).unsqueeze(0).permute(0 ,3 ,1 ,2).to(opt.device)
	with torch.no_grad():
		pred = net(inputs)
	pred = (pred.squeeze(0).permute(1 ,2 ,0)*256).int()
	print(pred.shape)
	print(torch.max(pred))
	print(torch.min(pred))
	cv2.imwrite("result.png", pred.numpy())
